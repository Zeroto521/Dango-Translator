...
class SwitchBtn(QWidget):
    ...
    def drawText(self, event, painter):
        ...
        if self.checked:
            ...
            painter.drawText(self.space, 0, self.width(
            ) / 2 + self.space * 2, self.height(), Qt.AlignCenter, self.textOn)
        else:
            painter.setPen(self.textColorOff)
            painter.drawText(self.width() / 2 - self.space * 2, 0, self.width() /
                             2 - self.space, self.height(), Qt.AlignCenter, self.textOff)
    ...
    def drawBg(self, event, painter):
        ...
        path.arcTo(QRectF(rect.left(), rect.top(),
                          circleWidth, circleWidth), 90, 180)
        path.lineTo(rect.width() - radius, rect.height())
        path.arcTo(QRectF(rect.width() - rect.height(),
                          rect.top(), circleWidth, circleWidth), 270, 180)
    ...
    def drawSlider(self, event, painter):
        ...
        sliderRect = QRect(self.startX + self.space,
                           self.space, sliderWidth, sliderWidth)
