...
class MainInterface(QMainWindow):
    ...
    def __init__(self, screen_scale_rate):
        ...
        self.rate = 1.25 if 1.01 <= screen_scale_rate <= 1.49 else 1
        self.get_settin()
        self.init_ui()
    ...
    def init_ui(self):
        ...
        self.translateText.setGeometry(
            0, 30*self.rate, 800*self.rate, 4000*self.rate)
        ...
        self.format.setTextOutline(
            QPen(QColor('#FF69B4'), 0.7, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
        self.translateText.mergeCurrentCharFormat(self.format)
        self.translateText.append("团子翻译器")
        ...
        self.StartButton = QPushButton(
            qtawesome.icon('fa.play', color='white'), "", self)
        self.StartButton.setIconSize(QSize(20, 20))
        self.StartButton.setGeometry(
            QRect(213*self.rate, 5*self.rate, 20*self.rate, 20*self.rate))
        self.StartButton.setToolTip(
            '<b>翻译 Translate</b><br>点击翻译（手动）<br>开始/停止（自动）')
        ...
        # 设置按钮
        self.SettinButton = QPushButton(
            qtawesome.icon('fa.cog', color='white'), "", self)
        self.SettinButton.setIconSize(QSize(20, 20))
        self.SettinButton.setGeometry(
            QRect(253*self.rate, 5*self.rate, 20*self.rate, 20*self.rate))
        ...
        # 范围按钮
        self.RangeButton = QPushButton(
            qtawesome.icon('fa.crop', color='white'), "", self)
        self.RangeButton.setIconSize(QSize(20, 20))
        self.RangeButton.setGeometry(
            QRect(293*self.rate, 5*self.rate, 20*self.rate, 20*self.rate))
        ...
        self.CopyButton = QPushButton(
            qtawesome.icon('fa.copy', color='white'), "", self)
        self.CopyButton.setIconSize(QSize(20, 20))
        self.CopyButton.setGeometry(
            QRect(333*self.rate, 5*self.rate, 20*self.rate, 20*self.rate))
        ...
        self.switchBtn.setGeometry(
            373*self.rate, 5*self.rate, 50*self.rate, 20*self.rate)
        ...
        self.MinimizeButton = QPushButton(
            qtawesome.icon('fa.minus', color='white'), "", self)
        self.MinimizeButton.setIconSize(QSize(20, 20))
        self.MinimizeButton.setGeometry(
            QRect(527*self.rate, 5*self.rate, 20*self.rate, 20*self.rate))
        ...
        # 退出按钮
        self.QuitButton = QPushButton(qtawesome.icon(
            'fa.times', color='white'), "", self)
        self.QuitButton.setIconSize(QSize(20, 20))
        self.QuitButton.setGeometry(
            QRect(567*self.rate, 5*self.rate, 20*self.rate, 20*self.rate))
    ...
    def leaveEvent(self, QEvent):
        ...
        try:
            ...
            self.SettinButton.setGeometry(
                QRect(width+40*self.rate, 5*self.rate, 20*self.rate, 20*self.rate))
            self.RangeButton.setGeometry(
                QRect(width+80*self.rate, 5*self.rate, 20*self.rate, 20*self.rate))
            self.CopyButton.setGeometry(
                QRect(width+120*self.rate, 5*self.rate, 20*self.rate, 20*self.rate))
            self.switchBtn.setGeometry(
                QRect(width+160*self.rate, 5*self.rate, 50*self.rate, 20*self.rate))
            self.MinimizeButton.setGeometry(
                QRect(width+314*self.rate, 5*self.rate, 20*self.rate, 20*self.rate))
            self.QuitButton.setGeometry(
                QRect(width+354*self.rate, 5*self.rate, 20*self.rate, 20*self.rate))
            self.translateText.setGeometry(
                0, 30*self.rate, self.width(), 4000*self.rate)
        ...
        if result["original"] and result["sign"]:
            ...
            if data["showOriginal"] == "True":
                if data["showColorType"] == "False":
                    self.format.setTextOutline(QPen(QColor(
                        data["fontColor"]["original"]), 0.7, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
                    self.translateText.mergeCurrentCharFormat(self.format)
                    self.translateText.append(result["original"])
                else:
                    self.translateText.append(
                        "<font color=%s>%s</font>" % (data["fontColor"]["original"], result["original"]))

            if result["caiyunPrivate"]:
                if data["showColorType"] == "False":
                    self.format.setTextOutline(QPen(QColor(
                        data["fontColor"]["caiyunPrivate"]), 0.7, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
                    self.translateText.mergeCurrentCharFormat(self.format)
                    self.translateText.append(result["caiyunPrivate"])
                else:
                    self.translateText.append(
                        "<font color=%s>%s</font>" % (data["fontColor"]["caiyunPrivate"], result["caiyunPrivate"]))

            if result["tencent"]:
                if data["showColorType"] == "False":
                    self.format.setTextOutline(QPen(QColor(
                        data["fontColor"]["tencent"]), 0.7, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
                    self.translateText.mergeCurrentCharFormat(self.format)
                    self.translateText.append(result["tencent"])
                else:
                    self.translateText.append(
                        "<font color=%s>%s</font>" % (data["fontColor"]["tencent"], result["tencent"]))

            if result["baidu"]:
                if data["showColorType"] == "False":
                    self.format.setTextOutline(QPen(QColor(
                        data["fontColor"]["baidu"]), 0.7, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
                    self.translateText.mergeCurrentCharFormat(self.format)
                    self.translateText.append(result["baidu"])
                else:
                    self.translateText.append(
                        "<font color=%s>%s</font>" % (data["fontColor"]["baidu"], result["baidu"]))

            if result["Bing"]:
                if data["showColorType"] == "False":
                    self.format.setTextOutline(QPen(QColor(
                        data["fontColor"]["Bing"]), 0.7, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
                    self.translateText.mergeCurrentCharFormat(self.format)
                    self.translateText.append(result["Bing"])
                else:
                    self.translateText.append(
                        "<font color=%s>%s</font>" % (data["fontColor"]["Bing"], result["Bing"]))

            if result["google"]:
                if data["showColorType"] == "False":
                    self.format.setTextOutline(QPen(QColor(
                        data["fontColor"]["google"]), 0.7, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
                    self.translateText.mergeCurrentCharFormat(self.format)
                    self.translateText.append(result["google"])
                else:
                    self.translateText.append(
                        "<font color=%s>%s</font>" % (data["fontColor"]["google"], result["google"]))

            if result["tencentweb"]:
                if data["showColorType"] == "False":
                    self.format.setTextOutline(QPen(QColor(
                        data["fontColor"]["tencentweb"]), 0.7, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
                    self.translateText.mergeCurrentCharFormat(self.format)
                    self.translateText.append(result["tencentweb"])
                else:
                    self.translateText.append(
                        "<font color=%s>%s</font>" % (data["fontColor"]["tencentweb"], result["tencentweb"]))

            if result["baiduweb"]:
                if data["showColorType"] == "False":
                    self.format.setTextOutline(QPen(QColor(
                        data["fontColor"]["baiduweb"]), 0.7, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
                    self.translateText.mergeCurrentCharFormat(self.format)
                    self.translateText.append(result["baiduweb"])
                else:
                    self.translateText.append(
                        "<font color=%s>%s</font>" % (data["fontColor"]["baiduweb"], result["baiduweb"]))

            if result["alapi"]:
                if data["showColorType"] == "False":
                    self.format.setTextOutline(QPen(QColor(
                        data["fontColor"]["ALAPI"]), 0.7, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
                    self.translateText.mergeCurrentCharFormat(self.format)
                    self.translateText.append(result["alapi"])
                else:
                    self.translateText.append(
                        "<font color=%s>%s</font>" % (data["fontColor"]["ALAPI"], result["alapi"]))

            if result["yeekit"]:
                if data["showColorType"] == "False":
                    self.format.setTextOutline(QPen(QColor(
                        data["fontColor"]["yeekit"]), 0.7, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
                    self.translateText.mergeCurrentCharFormat(self.format)
                    self.translateText.append(result["yeekit"])
                else:
                    self.translateText.append(
                        "<font color=%s>%s</font>" % (data["fontColor"]["yeekit"], result["yeekit"]))

            if result["jinshan"]:
                if data["showColorType"] == "False":
                    self.format.setTextOutline(QPen(QColor(
                        data["fontColor"]["jinshan"]), 0.7, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
                    self.translateText.mergeCurrentCharFormat(self.format)
                    self.translateText.append(result["jinshan"])
                else:
                    self.translateText.append(
                        "<font color=%s>%s</font>" % (data["fontColor"]["jinshan"], result["jinshan"]))

            if result["caiyun"]:
                if data["showColorType"] == "False":
                    self.format.setTextOutline(QPen(QColor(
                        data["fontColor"]["caiyun"]), 0.7, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
                    self.translateText.mergeCurrentCharFormat(self.format)
                    self.translateText.append(result["caiyun"])
                else:
                    self.translateText.append(
                        "<font color=%s>%s</font>" % (data["fontColor"]["caiyun"], result["caiyun"]))

            if result["youdao"]:
                if data["showColorType"] == "False":
                    self.format.setTextOutline(QPen(QColor(
                        data["fontColor"]["youdao"]), 0.7, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
                    self.translateText.mergeCurrentCharFormat(self.format)
                    self.translateText.append(result["youdao"])
                else:
                    self.translateText.append(
                        "<font color=%s>%s</font>" % (data["fontColor"]["youdao"], result["youdao"]))
